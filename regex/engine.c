/* engine.c -- This file is part of OS/0 libc.
...
#ifdef REDEBUG
static void print (struct match *m, char *caption, states st, int ch,
		   FILE * d);
#endif
#ifdef REDEBUG
...
dissect (register struct match *m, char *start, char *stop, sopno startst,
	 sopno stopst)
{
  register int i;
...
backref (register struct match *m, char *start, char *stop, sopno startst,
  ...
  for (ss = startst; !hard && ss < stopst; ss++)
	...
	if (((sp == m->beginp && !(m->eflags & REG_NOTBOL)) ||
	     (sp < m->endp && *(sp - 1) == '\n' &&
	      (m->g->cflags & REG_NEWLINE)) ||
	     (sp > m->beginp &&
	      !ISWORD (*(sp - 1)))) && (sp < m->endp && ISWORD (*sp)))
	  {			/* yes */
	  }
	else
	  return (NULL);
...
fast (register struct match *m, char *start, char *stop, sopno startst,
      sopno stopst)
{
...
slow (register struct match *m, char *start, char *stop, sopno startst,
      sopno stopst)
{
  register states st = m->st;
 ...
 */
static states
step (register struct re_guts *g, sopno start, sopno stop, register states bef,
      int ch, register states aft)
{
  register cset *cs;
  register sop s;
  register sopno pc;
  ...
  for (pc = start, INIT (here, pc); pc != stop; pc++, INC (here))
	...
	case OOR1:		/* done a branch, find the O_CH */
	  if (ISSTATEIN (aft, here))
	    {
	      for (look = 1;
		   OP (s = g->strip[pc + look]) != O_CH; look += OPND (s))
		assert (OP (s) == OOR2);
	      FWD (aft, aft, look);
	    }
	  break;
